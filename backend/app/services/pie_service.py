...
class PieService:
    ...
    async def get_by_id(self, pie_id: UUID, user_id: UUID) -> Optional[Pie]:
        """Get a pie by ID, ensuring it belongs to the user."""
        query = (
            select(Pie)
            .options(selectinload(Pie.slices))
            .where(Pie.id == pie_id, Pie.user_id == user_id)
        )
    ...
    async def create(
        ...
        max_order_query = select(Pie.display_order).where(Pie.user_id == user_id).order_by(Pie.display_order.desc()).limit(1)
    ...
    async def update(
        ...
        """Update a pie."""
        pie = await self.get_by_id(pie_id, user_id)
        if not pie:
            return None
    ...
    async def delete(self, pie_id: UUID, user_id: UUID) -> bool:
        """Delete a pie (and all its slices via cascade)."""
        query = delete(Pie).where(Pie.id == pie_id, Pie.user_id == user_id)
        result = await self.db.execute(query)
        return result.rowcount > 0

    async def reorder(self, user_id: UUID, pie_ids: List[UUID]) -> bool:
    ...
    async def get_total_allocation(self, user_id: UUID) -> Decimal:
        """Get total allocation percentage across all active pies."""
        pies = await self.get_all_by_user(user_id)
